...
const Metric = ({ label, value, helper, tone = 'neutral' }) => {
  const tones = {
    neutral: 'bg-white text-editorial-charcoal ring-[#e5e7eb]',
...
const VoucherRealtimePreview = ({ preview, className = '' }) => {
  ...
  return (
    <section className={`rounded-2xl border border-[#e5e7eb] bg-editorial-paper p-4 ${className}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[10px] font-bold uppercase text-amber-700">Preview real-time</div>
          <h3 className="mt-1 text-base font-bold text-editorial-charcoal">Simulasi aturan voucher</h3>
        ...
        {preview.rules.map((rule) => (
          <div key={rule.key} className="flex items-center justify-between gap-3 rounded-2xl bg-white px-3 py-2 text-xs font-semibold ring-1 ring-[#e5e7eb]">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate font-bold text-editorial-charcoal">{rule.label}</span>
            </div>
