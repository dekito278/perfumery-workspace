...
const PaginationOrLoadMore = ({ visibleCount, totalCount, onLoadMore, loading = false }) => {
  if (visibleCount >= totalCount) {
    return totalCount ? (
      <div className="py-2 text-center text-[11px] font-semibold text-[#6b7280]">Showing {totalCount} of {totalCount}</div>
    ) : null;
  }

  return (
    <div className="grid grid-cols-[1fr_auto] items-center gap-2 py-2">
      <div className="text-[11px] font-semibold text-[#6b7280]">Showing 1-{visibleCount} of {totalCount}</div>
