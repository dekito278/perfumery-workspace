// Scrapers for the three material reference sources. Lifted out of the Vite dev plugin so the serverless
// endpoints and the dev middleware run the same code — the plugin was the ONLY implementation, and it is
// loaded only when isDev, so every import button 404'd in production (audit round 8).
// Runtime-agnostic: fetch + string parsing only, no Vite and no browser APIs.
...
export { importScentreeByUrl, importPerfumersWorldByUrl, importTgscByUrl };
