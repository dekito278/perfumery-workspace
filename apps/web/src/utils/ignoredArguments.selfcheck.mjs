...
// happening.
//
// That is not a hypothetical. formatDate took one argument and three buyer-facing screens were calling
// formatDate(value, t) — handing it the translator so dates would follow the shop's language. Fourteen
// calls across four files, every one of them looking finished, every one printing Indonesian month names
// to an English reader. The fix had been written and never wired up, and nothing could see it, because
// the only evidence was an argument going nowhere.
//
// It also made fixing it dangerous: turning that second parameter into a real one would have passed a
// function to toLocaleDateString and blanked the page.
//
