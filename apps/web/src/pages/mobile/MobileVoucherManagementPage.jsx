    ...
    <MobileAuthenticatedLayout showFab={false}>
      ...
      <main className="mobile-page space-y-4">
        ...
        <section className="mobile-card space-y-4 p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h2 className="text-base font-bold text-editorial-charcoal">Analytics voucher</h2>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-2xl bg-editorial-paper px-3 py-2">
              ...
              <div className="mt-1 text-lg font-bold text-editorial-charcoal">{voucherAnalytics.totalUsage}</div>
            </div>
            ...
            <div className="rounded-2xl bg-white px-3 py-2 ring-1 ring-[#e5e7eb]">
              ...
              <div className="mt-1 text-sm font-bold text-editorial-charcoal">{formatTotal(voucherAnalytics.revenueBeforeDiscount)}</div>
            </div>
            <div className="rounded-2xl bg-editorial-ivory px-3 py-2">
              <div className="text-[10px] font-bold uppercase text-editorial-charcoal">Sesudah diskon</div>
              <div className="mt-1 text-sm font-bold text-editorial-charcoal">{formatTotal(voucherAnalytics.revenueAfterDiscount)}</div>
            </div>
          </div>
          <div className="grid gap-3">
            <div>
              ...
              <div className="mt-2 grid gap-2">
                {voucherAnalytics.topVouchers.slice(0, 3).map((item, index) => (
                  <div key={item.key} className="flex items-center justify-between gap-2 rounded-2xl bg-editorial-paper px-3 py-2 text-xs font-bold">
                    <span className="min-w-0 truncate">#{index + 1} {item.label}</span>
                    <span className="shrink-0 text-editorial-charcoal">{item.count}x</span>
                  </div>
                ))}
              ...
              {voucherStatusFilters.map((filter) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-10 shrink-0 rounded-2xl border px-3 text-xs font-bold ${selected ? 'border-editorial-charcoal bg-editorial-charcoal text-white' : 'border-[#e5e7eb] bg-white text-editorial-charcoal'}`}
                  >
          ...
          {visibleVouchers.map((voucher) => {
            ...
            return (
              <article key={voucher.id || voucher.code} className="mobile-card space-y-3 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="truncate text-lg font-bold tracking-[0.08em] text-editorial-charcoal">{voucher.code}</h3>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ${status.className}`}>{status.label}</span>
                      <span className="rounded-full bg-editorial-paper px-2.5 py-1 text-[10px] font-bold uppercase text-editorial-charcoal">{voucher.discountType}</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-semibold text-editorial-charcoal">
                  ...
                  <div className="col-span-2 rounded-2xl bg-editorial-paper px-3 py-2">
        ...
        <section className="space-y-3">
          <div className="mobile-card space-y-3 p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h2 className="text-base font-bold text-editorial-charcoal">Laporan penggunaan</h2>
          ...
          {visibleUsageReport.map((entry) => (
            <article key={entry.id || `${entry.voucherCode}-${entry.orderNumber}-${entry.usedAt}`} className="mobile-card space-y-3 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="truncate text-base font-bold tracking-[0.08em] text-editorial-charcoal">{entry.voucherCode}</h3>
                  <p className="mt-1 text-xs font-semibold text-[#6b7280]">{entry.orderNumber || '-'}</p>
                </div>
                <div className="shrink-0 rounded-2xl bg-editorial-paper px-3 py-2 text-right text-xs font-bold text-editorial-charcoal">
