    ...
    <MobileAuthenticatedLayout>
      ...
      <div className="mobile-page-shell">
        <MobileTopBar
          ...
          action={<Truck className="h-5 w-5 text-[#263d27]" />}
        />
        ...
        <form onSubmit={handleSave} className="grid gap-3 pb-24">
          <section className="mobile-commerce-panel p-4">
            <button
              ...
              className={`mobile-interactive mobile-pressable flex h-12 w-full items-center justify-center gap-2 rounded-2xl text-sm font-bold ${
                settings.enabled ? 'bg-[#263d27] text-white' : 'bg-[#eef2e8] text-[#51624b]'
              }`}
            >
              <BadgePercent className="h-4 w-4" />
              {settings.enabled ? 'Promo ongkir aktif' : 'Promo ongkir nonaktif'}
            </button>
            <p className="mt-3 text-xs font-semibold leading-5 text-[#51624b]">{preview}</p>
            <p className="mt-2 text-[11px] font-bold leading-5 text-[#6f7d61]">
              Area Jawa dibaca dari provinsi tujuan: Banten, DKI Jakarta, Jawa Barat, Jawa Tengah, DI Yogyakarta, dan Jawa Timur.
            </p>
          </section>

          <section className="mobile-commerce-panel p-4">
            <div className="text-xs font-bold uppercase text-[#6f7d61]">Pola promo</div>
          ...
          <section className="mobile-commerce-panel grid gap-3 p-4">
            <label className="grid gap-2">
              <span className="text-[10px] font-bold uppercase text-[#6f7d61]">Minimal belanja</span>
              <input
                type="number"
                min="0"
                step="1000"
            ...
            <label className={`grid gap-2 ${needsJavaAmount ? '' : 'opacity-50'}`}>
              <span className="text-[10px] font-bold uppercase text-[#6f7d61]">Maksimal Pulau Jawa</span>
              <input
                type="number"
                min="0"
                step="1000"
            ...
            <label className={`grid gap-2 ${needsOtherAmount ? '' : 'opacity-50'}`}>
              <span className="text-[10px] font-bold uppercase text-[#6f7d61]">Diskon luar Jawa / semua area</span>
              <input
                type="number"
                min="0"
                step="1000"
          ...
          <section className="mobile-commerce-panel grid gap-3 p-4">
            <div className="text-xs font-bold uppercase text-[#6f7d61]">Periode</div>
            <label className="grid gap-2">
              <span className="text-[10px] font-bold uppercase text-[#6f7d61]">Mulai</span>
              <input
                type="date"
            ...
            <label className="grid gap-2">
              <span className="text-[10px] font-bold uppercase text-[#6f7d61]">Selesai</span>
              <input
                type="date"
          ...
          <section className="mobile-commerce-panel p-4 text-xs font-bold text-[#263d27]">
          ...
          <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#263d27]/10 bg-white/95 p-3 backdrop-blur">
