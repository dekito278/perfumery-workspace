          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {collections.map((item) => (
              <button
                ...
                className={`min-h-[48px] rounded-2xl border px-3 py-2 text-left text-xs font-bold ${activeCollection === item.key ? 'border-editorial-charcoal bg-editorial-ivory text-editorial-charcoal' : 'border-[#e5e7eb] bg-white text-[#6b7280]'}`}
              >
                {item.title}
              </button>
            ))}
          ...
          {options.map((option) => (
            <article key={option.id} className="mobile-card p-3">
              <div className="flex items-start justify-between gap-3">
                {option.imageUrl ? <img src={option.imageUrl} alt={option.label} className="h-16 w-16 shrink-0 rounded-2xl object-cover" loading="lazy" decoding="async" width="64" height="64" /> : null}
                <div className="min-w-0 flex-1">
