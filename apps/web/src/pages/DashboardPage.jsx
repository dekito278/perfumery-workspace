...
const DashboardPage = () => {
  ...
  const actionCenterItems = [
    ...
    },
    {
      ...
      action: 'Cek produk',
      onClick: () => navigate('/studio/products?filter=lowstock'),
              ...
              return (
                <section key={item.key} className={`rounded-2xl border p-4 shadow-sm ${item.tone}`}>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-2xl bg-white/85 px-3 py-1 text-xl font-bold text-editorial-charcoal">{item.count}</span>
                  </div>
                  <h2 className="mt-4 text-base font-bold text-editorial-charcoal">{item.label}</h2>
                  ...
                  <div className="mt-3 grid gap-2">
                    {item.rows.map((row) => (
                      <button key={row.id} type="button" onClick={row.onClick} className="rounded-2xl bg-white/85 px-3 py-2 text-left transition hover:bg-white">
                        <div className="truncate text-xs font-bold text-editorial-charcoal">{row.title}</div>
                ...
                return (
                  <button key={log.id} type="button" onClick={() => navigate(`/studio/orders/${log.orderId || log.orderNumber}`)} className={`rounded-2xl border p-4 text-left ${important ? 'border-amber-200 bg-amber-50' : 'bg-[#fbfaf7]'}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-bold uppercase text-editorial-charcoal">{auditActionLabels[log.action] || log.action}</span>
                    </div>
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                      {changes.slice(0, 2).map((change) => (
                        <div key={change.key} className="rounded-xl bg-white px-3 py-2 text-xs font-semibold">
                          <div className="font-bold uppercase text-editorial-charcoal">{change.label}</div>
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_0.9fr]">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="mt-1 text-xl font-bold text-editorial-charcoal">
                  ...
                  {opsHealth.syncQueue.slice(0, 3).map((item) => (
                    <div key={item.id || item.orderNumber} className="rounded-2xl border border-white bg-white/90 px-4 py-3 text-sm font-semibold">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <span className="font-bold text-editorial-charcoal">{item.orderNumber}</span>
            ...
            <section className="rounded-2xl border bg-white/90 p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="rounded-full bg-editorial-ivory px-3 py-1 text-xs font-bold uppercase text-editorial-charcoal">
