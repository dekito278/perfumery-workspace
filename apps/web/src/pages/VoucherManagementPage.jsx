      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
          <form onSubmit={submitVoucher} className="rounded-2xl border bg-white/90 p-5 shadow-sm">
            ...
            <Tabs defaultValue="rules" className="mt-5">
              <TabsList className="grid h-auto w-full grid-cols-3 rounded-2xl bg-editorial-paper p-1">
              ...
              <TabsContent value="eligibility" className="mt-5 grid gap-3">
                ...
                <p className="rounded-2xl bg-editorial-paper px-4 py-3 text-xs font-semibold leading-relaxed text-muted-foreground">
                    ...
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-lg font-bold tracking-[0.08em] text-editorial-charcoal">{voucher.code}</h3>
                          <span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase ${status.className}`}>{status.label}</span>
                          <span className="rounded-full bg-white px-3 py-1 text-[10px] font-bold uppercase text-editorial-charcoal">{voucher.discountType}</span>
                        </div>
        ...
        <section className="mt-6 rounded-2xl border bg-white/90 p-5 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="rounded-2xl bg-editorial-ivory px-4 py-2 text-xs font-bold text-editorial-charcoal">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-2xl border border-editorial-charcoal/10 bg-editorial-paper p-4">
              ...
              <div className="mt-1 text-3xl font-bold text-editorial-charcoal">{voucherAnalytics.totalUsage}</div>
            </div>
            <div className="rounded-2xl border border-editorial-charcoal/10 bg-white p-4">
              ...
              <div className="mt-1 text-2xl font-bold text-editorial-charcoal">{formatTotal(voucherAnalytics.revenueBeforeDiscount)}</div>
            </div>
            <div className="rounded-2xl border border-editorial-charcoal/10 bg-editorial-ivory p-4">
              ...
              <div className="mt-1 text-2xl font-bold text-editorial-charcoal">{formatTotal(voucherAnalytics.revenueAfterDiscount)}</div>
            </div>
              ...
              <div className="mt-3 grid gap-2">
                {voucherAnalytics.topVouchers.slice(0, 5).map((item, index) => (
                  <div key={item.key} className="flex items-center justify-between gap-3 rounded-2xl bg-white px-3 py-2 text-sm font-semibold">
                    ...
                    <span className="shrink-0 text-right text-editorial-charcoal">{item.count}x / {formatTotal(item.discountTotal)}</span>
                  </div>
                ))}
            ...
            <div className="rounded-2xl border bg-[#fbfaf7] p-4">
              ...
              <div className="mt-3 grid gap-2">
                {voucherAnalytics.topCustomers.slice(0, 5).map((item, index) => (
                  <div key={item.key} className="flex items-center justify-between gap-3 rounded-2xl bg-white px-3 py-2 text-sm font-semibold">
                    <span className="min-w-0 truncate"><span className="mr-2 text-muted-foreground">#{index + 1}</span>{item.label}</span>
                    <span className="shrink-0 text-right text-editorial-charcoal">{item.count}x / {formatTotal(item.revenueAfterDiscount)}</span>
                  </div>
                ))}
        ...
        <section className="mt-6 rounded-2xl border bg-white/90 p-5 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <div className="rounded-2xl bg-editorial-paper px-4 py-2 text-xs font-bold text-editorial-charcoal">
          ...
          {filteredUsageReport.length ? (
            <div className="mt-5 overflow-hidden rounded-2xl border">
              <table className="min-w-full divide-y text-sm">
                <thead className="bg-editorial-paper text-left text-[11px] font-bold uppercase text-muted-foreground">
                  <tr>
                ...
                <tbody className="divide-y bg-white">
                  {filteredUsageReport.map((entry) => (
                    <tr key={entry.id || `${entry.voucherCode}-${entry.orderNumber}-${entry.usedAt}`}>
                      <td className="px-4 py-3 font-bold tracking-[0.08em] text-editorial-charcoal">{entry.voucherCode}</td>
                      <td className="px-4 py-3 font-semibold text-editorial-charcoal">{entry.orderNumber || '-'}</td>
                      <td className="px-4 py-3">
                        <div className="font-semibold text-editorial-charcoal">{entry.customerName || '-'}</div>
                        {entry.customerCode ? <div className="text-xs font-semibold text-muted-foreground">{entry.customerCode}</div> : null}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-editorial-charcoal">
