-- The synthetic `voucher_discount` line item (slug `voucher-<code>`, no product) was only skipped by
-- the client-side `isStockOrderItem`, not by this RPC — so the RPC raised "Product ... is unavailable"
-- on every catalog order that used a voucher. The live client path then silently fell back to a
-- non-atomic JS deduction (oversell risk on discounted items); the authoritative endpoint auto-cancelled
-- the order outright. Skip `voucher_discount` here too so voucher orders deduct atomically.
-- Only change vs 20260508211500: the type-skip clause now matches both synthetic line types.
...
begin
    ...
    loop
        if line_item->>'type' in ('bespoke_request', 'voucher_discount') then
            continue;
        end if;
