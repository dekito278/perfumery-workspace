alter table public.storefront_orders
    add column if not exists bespoke_production_status text,
    add column if not exists bespoke_production_timeline jsonb not null default '[]'::jsonb;

alter table public.storefront_orders
    drop constraint if exists storefront_orders_bespoke_production_status_check;

alter table public.storefront_orders
    add constraint storefront_orders_bespoke_production_status_check check (
        bespoke_production_status is null
        or bespoke_production_status in ('review_brief', 'formula', 'sample', 'approval', 'production', 'ready')
    );

alter table public.storefront_orders
    drop constraint if exists storefront_orders_bespoke_production_timeline_array;

alter table public.storefront_orders
    add constraint storefront_orders_bespoke_production_timeline_array check (
        jsonb_typeof(bespoke_production_timeline) = 'array'
    );

create index if not exists storefront_orders_bespoke_production_status_idx
    on public.storefront_orders (bespoke_production_status);
